...
func makeNquad(sub, pred string, val *intern.Value) *intern.NQuad {
	return &intern.NQuad{
		Subject:     sub,
		Predicate:   pred,
		ObjectValue: val,
...
func makeNquadEdge(sub, pred, obj string) *intern.NQuad {
	return &intern.NQuad{
		Subject:   sub,
		Predicate: pred,
		ObjectId:  obj,
...
func TestNquadsFromJson1(t *testing.T) {
	...
	oval := &intern.Value{&intern.Value_StrVal{"Alice"}}
	require.Contains(t, nq, makeNquad("_:blank-0", "name", oval))

	oval = &intern.Value{&intern.Value_DoubleVal{26}}
	require.Contains(t, nq, makeNquad("_:blank-0", "age", oval))

	oval = &intern.Value{&intern.Value_BoolVal{true}}
	require.Contains(t, nq, makeNquad("_:blank-0", "married", oval))

	oval = &intern.Value{&intern.Value_StrVal{tn.Format(time.RFC3339Nano)}}
...
func TestNquadsFromJson2(t *testing.T) {
	...
	oval := &intern.Value{&intern.Value_StrVal{"Charlie"}}
	require.Contains(t, nq, makeNquad("_:blank-1", "name", oval))

	oval = &intern.Value{&intern.Value_BoolVal{false}}
	require.Contains(t, nq, makeNquad("_:blank-1", "married", oval))

	oval = &intern.Value{&intern.Value_StrVal{"Bob"}}
...
func TestNquadsFromJson3(t *testing.T) {
	...
	oval := &intern.Value{&intern.Value_StrVal{"Wellington Public School"}}
	require.Contains(t, nq, makeNquad("_:blank-1", "Name", oval))
}
...
func checkCount(t *testing.T, nq []*intern.NQuad, pred string, count int) {
	for _, n := range nq {
		if n.Predicate == pred {
...
func TestParseNQuads(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []*intern.NQuad{
		makeNquad("_:a", "predA", &intern.Value{&intern.Value_DefaultVal{"A"}}),
		makeNquad("_:b", "predB", &intern.Value{&intern.Value_DefaultVal{"B"}}),
...
func TestParseNQuadsDelete(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []*intern.NQuad{
		makeNquad("_:a", x.Star, &intern.Value{&intern.Value_DefaultVal{x.Star}}),
	}, nqs)
}
