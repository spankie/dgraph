...
// ObjectValue converts into intern.Value.
func ObjectValue(id TypeID, value interface{}) (*intern.Value, error) {
	def := &intern.Value{&intern.Value_StrVal{""}}
	var ok bool
	...
	case StringID:
		...
		return &intern.Value{&intern.Value_StrVal{v}}, nil
	case DefaultID:
		var v string
		if v, ok = value.(string); !ok {
			return def, x.Errorf("Expected value of type string. Got : %v", value)
		}
		return &intern.Value{&intern.Value_DefaultVal{v}}, nil
	case IntID:
		var v int64
		if v, ok = value.(int64); !ok {
			return def, x.Errorf("Expected value of type int64. Got : %v", value)
		}
		return &intern.Value{&intern.Value_IntVal{v}}, nil
	case FloatID:
		var v float64
		if v, ok = value.(float64); !ok {
			return def, x.Errorf("Expected value of type float64. Got : %v", value)
		}
		return &intern.Value{&intern.Value_DoubleVal{v}}, nil
	case BoolID:
		var v bool
		if v, ok = value.(bool); !ok {
			return def, x.Errorf("Expected value of type bool. Got : %v", value)
		}
		return &intern.Value{&intern.Value_BoolVal{v}}, nil
	case BinaryID:
		var v []byte
		if v, ok = value.([]byte); !ok {
			return def, x.Errorf("Expected value of type []byte. Got : %v", value)
		}
		return &intern.Value{&intern.Value_BytesVal{v}}, nil
	...
	case GeoID:
		...
		return &intern.Value{&intern.Value_GeoVal{b}}, nil
	case DateTimeID:
		b, err := toBinary(id, value)
		if err != nil {
			return def, err
		}
		return &intern.Value{&intern.Value_DatetimeVal{b}}, nil
	case PasswordID:
		var v string
		if v, ok = value.(string); !ok {
			return def, x.Errorf("Expected value of type password. Got : %v", value)
		}
		return &intern.Value{&intern.Value_PasswordVal{v}}, nil
	default:
